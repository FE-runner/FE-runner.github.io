...
function hoverDir(obj, ev) {
  var x = getPos(obj).left + obj.offsetWidth / 2 - ev.clientX; //鼠标位置与obj中心的横轴距离
  var y = getPos(obj).top + obj.offsetHeight / 2 - ev.clientY; //鼠标位置与obj中心的纵轴距离
...
function slideInOutFull(obj) {
  obj.onmouseenter = function(ev) {
    ...
    move(oSpan, { left: 0, top: m / 2 });
  };
  obj.onmouseleave = function(ev) {
    var oEvent = ev || event;
    oEvent.cancelBubble = true;
    var n = hoverDir(this, oEvent);
    var m = this.offsetWidth * 1.5;

    var oSpan = this.children[1];
    switch (n) {
      case 0:
        move(oSpan, { left: m, top: 0 });
        break;
      case 1:
        move(oSpan, { left: 0, top: m });
        break;
      case 2:
        move(oSpan, { left: -m, top: 0 });
        break;
